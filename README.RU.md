...
Совет: у формы запуска есть постоянный адрес — `https://github.com/ВАШ_ЛОГИН/ВАШ_РЕПОЗИТОРИЙ/actions/workflows/download.yml` (добавьте в закладки). Отметьте в форме **publish**, чтобы прикрепить файлы к GitHub **Release** — постоянной странице с прямыми ссылками на скачивание (см. *Releases* в репозитории или ссылку в сводке запуска). Это правильный способ хранить файлы «в репозитории»: коммитить видео в git-историю нельзя — она распухнет, а GitHub и так блокирует файлы больше 100 МБ.
