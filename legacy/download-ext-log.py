...
def main():
    ...
    if len(args) >= 2:
        url = args[0]
        quality = args[1]
        lang = args[2] if len(args) >= 3 else DEFAULT_LANG
    else:
        url = input("🔗 Ссылка на YouTube: ").strip()
        print("\nЧто скачать?")
        print("  1 — видео максимального качества (1080p+ через DASH)")
        print("  2 — видео среднего качества")
        print("  3 — видео низкого качества")
        print("  4 — только аудио (mp3) для прослушивания в пути")
        choice = input("Введите номер (1/2/3/4) [по умолчанию 1]: ").strip() or "1"
        quality = {"1": "max", "2": "medium", "3": "low", "4": "audio"}.get(choice, "max")

        lang_input = input(f"\nЯзык аудио/транскрипции [{DEFAULT_LANG}]: ").strip()
        lang = lang_input or DEFAULT_LANG

        if quality != "audio":
            mp3_ans = input("Сохранить также mp3-дорожку? (y/N): ").strip().lower()
            save_mp3 = mp3_ans in ("y", "yes", "д", "да")

    download_video(url, quality, lang=lang, save_mp3=save_mp3)
